...
qemu-system-aarch64 -machine virt,virtualization=on -cpu cortex-a72 -m 64M -nographic -semihosting -kernel bin/kernel.elf -drive file=disk.img,if=none,format=raw,id=hd0 -device virtio-blk-device,drive=hd0
