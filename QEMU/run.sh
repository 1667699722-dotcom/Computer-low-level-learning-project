...
qemu-system-aarch64 -machine virt,virtualization=on,gic-version=2 -cpu cortex-a72  -smp 2 -m 512M -nographic -semihosting -kernel bin/kernel.elf -drive file=disk.img,if=none,format=raw,id=hd -device virtio-blk-device,drive=hd -global virtio-mmio.force-legacy=false
